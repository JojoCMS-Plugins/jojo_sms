...
class JOJO_Plugin_Jojo_sms extends JOJO_Plugin
{
    ...
    function _getContent()
    {
        ...
        $price = Jojo::getOption('clickatell-price'); //12 cents per message
        ...
        if ($submit != '') {
          ...
          if (count($errors) == 0) {
              foreach (Jojo::listPlugins('/external/sms-api/sms_api.php') as $pluginfile) {require_once($pluginfile);}
              $sms = new sms(Jojo::getOption('clickatell-api'),Jojo::getOption('clickatell-username'),Jojo::getOption('clickatell-password'));
        ...
        $content['content'] = $smarty->fetch('jojo_sms.tpl');
        return $content;
    }
}